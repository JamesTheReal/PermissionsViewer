...
module.exports = (() => {
    ...
    const {Patcher, DiscordModules, DiscordModules: { React, DiscordConstants: { ChannelTypes } }, WebpackModules, PluginUtilities, Toasts, DiscordClasses, Utilities, DOMTools, ColorConverter, DCM, Structs, ReactTools} = Api;
...
#permissions-modal .perm-item {
    ...
    <ul class="member-perms \${root} \${rolesList} \${endBodySection}"></ul>
        ...
        async patchChannelContextMenu() {
            const buildUserContextMenu = function (channel, _this ,isDM = false, menu = true) {
                return (menu ? ContextMenu.buildMenu : ContextMenu.buildItems).call(ContextMenu, [
                    {
                        label: "Permissions",
                        id: "permissions-channel",
                        action: () => {
                            const GUILD_ID = SelectedGuildStore.getGuildId();
                            const guild = GuildStore.getGuild(GUILD_ID);
                            _this.showModal(_this.createModalChannel(channel.name, channel, guild));
                        }
                    }
                ]);
            };

            // DCM.getDiscordMenu("useChannelMarkAsReadItem").then(UserRolesItems => {
            //     if (this.promises.cancelled) return;

            //     Patcher.after(UserRolesItems, "default", (_, [channel], ret) => {
            //         if (!channel) return;

            //         return [
            //             ret,
            //             buildUserContextMenu(channel, this, false, false),
            //             React.createElement(MenuSeparator, null)
            //         ];
            //     });
            // });

            DCM.getDiscordMenu("useChannelDeleteItem").then(UserRolesItems => {
                if (this.promises.cancelled) return;
        ...
        async patchUserContextMenu() {
            const buildUserContextMenu = function (user, _this ,isDM = false, menu = true) {
                return (menu ? ContextMenu.buildMenu : ContextMenu.buildItems).call(ContextMenu, [
                    {
                        label: "Permissions",
                        id: "permissions-user",
                        action: () => {
                            ...
                            const GUILD_ID = SelectedGuildStore.getGuildId();
                            const guild = GuildStore.getGuild(GUILD_ID);
                            const member = MemberStore.getMember(GUILD_ID, user.id);
                            _this.showModal(_this.createModalUser(name, member, guild));
                        }
                    }
                ]);
            };
            DCM.getDiscordMenu("useUserProfileItem").then(UserRolesItems => {
                if (this.promises.cancelled) return;

                Patcher.after(UserRolesItems, "default", (_, [userId], ret) => {
                    const user = UserStore.getUser(userId);
                    const GUILD_ID = SelectedGuildStore.getGuildId();
                    if (!user || !GUILD_ID) return;
                    return [
                        ret,
                        buildUserContextMenu(user, this, false, false),
                        React.createElement(MenuSeparator, null)
                    ];
                });
            });
